...
use Modules\Events\Models\TagTranslation;
...
class TagController extends AdminController
{
    ...
    public function index(Request $request)
    {
        ...
        return view('Events::admin.tag.index', $data);
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        $row = Tag::find($id);
        if (empty($row)) {
            return redirect('admin/module/events/tag');
        }
        ...
        return view('Events::admin.tag.detail', $data);
    }
    ...
    public function store(Request $request, $id){
        ...
        if($id>0){
            $row = Tag::find($id);
            if (empty($row)) {
                return redirect(route('events.admin.tag.index'));
            }
        }else{
            $row = new Tag();
//            $row->status = "publish";
        ...
        if ($res) {
            if($id > 0 ){
                ...
                return redirect(route('events.admin.tag.index'))->with('success', __('Tag Created') );
            }
        }
    }
