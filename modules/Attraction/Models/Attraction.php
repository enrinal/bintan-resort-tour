<?php
    ...
    class Attraction extends Bookable
    {
        use SoftDeletes;
        protected $table                              = 'bravo_attractions';
        public    $checkout_booking_detail_file       = 'Attraction::frontend/booking/detail';
        public    $checkout_booking_detail_modal_file = 'Attraction::frontend/booking/detail-modal';
        public    $email_new_booking_file             = 'Attraction::emails.new_booking_detail';
        public    $type                               = 'attraction';
        protected $fillable                           = [
            //Attraction info
            'title',
            'slug',
         ...
         */
        public function category_attraction()
        {
            return $this->hasOne("Modules\Attraction\Models\AttractionCategory", "id", 'category_id')->with(['translations']);
        }
        ...
        {
            $meta['seo_title'] = __("Search for Attractions");
            if (!empty($title = setting_item_with_lang("attraction_page_list_seo_title", false))) {
                $meta['seo_title'] = $title;
            } else if (!empty($title = setting_item_with_lang("attraction_page_search_title"))) {
                $meta['seo_title'] = $title;
            }
            $meta['seo_image'] = null;
            if (!empty($title = setting_item("attraction_page_list_seo_image"))) {
                $meta['seo_image'] = $title;
            } else if (!empty($title = setting_item("attraction_page_search_banner"))) {
                $meta['seo_image'] = $title;
            }
            $meta['seo_desc'] = setting_item_with_lang("attraction_page_list_seo_desc");
            $meta['seo_share'] = setting_item_with_lang("attraction_page_list_seo_share");
            $meta['full_url'] = url(config('attraction.attraction_route_prefix'));
            return $meta;
        }
        ...
        {
            return $this->hasOne($this->attractionMetaClass, "attraction_id");
        }
         ...
         */
        public function attraction_term()
        {
            return $this->hasMany($this->attractionTermClass, "attraction_id");
        }
        ...
        public function getDetailUrl($include_param = true)
        {
            ...
            $urlDetail = app_get_locale(false, false, '/') . config('attraction.attraction_route_prefix') . "/" . $this->slug;
            if(!empty($param)){
        ...
        public static function getLinkForPageSearch($locale = false, $param = [])
        {
            ...
            return url(app_get_locale(false, false, '/') . config('attraction.attraction_route_prefix') . "?" . http_build_query($param));
        }
        ...
        {
            return url('admin/module/attraction/edit/' . $this->id);
        }
        ...
        {
            $meta = $this->attractionMetaClass::where('attraction_id', $this->id)->first();
            if (!$meta) {
                $meta = new $this->attractionMetaClass();
                $meta->attraction_id = $this->id;
            }
        ...
        public function addToCart(Request $request)
        {
            ...
            $list_fees = setting_item('attraction_booking_buyer_fees');
            if (!empty($list_fees)) {
        ...
        public function getDataPriceAvailabilityInRanges($start_date){
            $datesRaw = $this->attractionDateClass::getDatesInRanges($start_date,$this->id);
            $dates = [
                'base_price' => null,
                'person_types' => null,
            ];
        ...
        {
            $attractionDate = $this->attractionDateClass::where('target_id', $this->id)->where('start_date', $start_date)->where('active', 1)->first();
            ...
            $maxGuests = !empty($attractionDate->max_guests) ? $attractionDate->max_guests : $this->max_people;
            ...
            if ($meta) {
                ...
                if ($meta->enable_open_hours) {
                    ...
                    if (!isset($open_hours[$nDate]) or empty($open_hours[$nDate]['enable'])) {
                        $this->sendError(__("This attraction is not open on your selected day"));
                    }
                }
            }
        ...
        public function getBookingData()
        {
            ...
            $list_fees = setting_item_array('attraction_booking_buyer_fees');
            if(!empty($list_fees)){
        ...
        {
            return setting_item("attraction_enable_review", 0);
        }

        public function getReviewApproved()
        {
            return setting_item("attraction_review_approved", 0);
        }

        public function check_enable_review_after_booking()
        {
            $option = setting_item("attraction_enable_review_after_booking", 0);
            if ($option) {
        ...
        {
            $reviewStats = [];
            if (!empty($list = setting_item("attraction_review_stats", []))) {
        ...
        public function getReviewDataAttribute()
        {
            ...
            $dataTotalReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id', $this->id)->where('object_model', "attraction")->where("status", "approved")->first();
         ...
         * Get Score Review
         *
         * Using for loop attraction
         */
        public function getScoreReview()
        {
            $attraction_id = $this->id;
            $list_score = Cache::rememberForever('review_' . $this->type . '_' . $attraction_id, function () use ($attraction_id) {
                $dataReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id', $attraction_id)->where('object_model', "attraction")->where("status", "approved")->first();
                return [
        ...
        public function getNumberServiceInLocation($location)
        {
            ...
            if (!empty($location)) {
                $number = parent::join('bravo_locations', function ($join) use ($location) {
                    $join->on('bravo_locations.id', '=', 'bravo_attractions.location_id')->where('bravo_locations._lft', '>=', $location->_lft)->where('bravo_locations._rgt', '<=', $location->_rgt);
                })->where("bravo_attractions.status", "publish")->with(['translations'])->count("bravo_attractions.id");
            }

            if(empty($number)) return false;
            if ($number > 1) {
                return __(":number Attractions", ['number' => $number]);
            }
            return __(":number Attraction", ['number' => $number]);
        }
        ...
        public function getBookingsInRange($from, $to)
        {
            ...
            $query->where('object_model', 'attraction');
        ...
        public function saveCloneByID($clone_id)
        {
            ...
            $selected_terms = $old->attraction_term->pluck('term_id');
            ...
            foreach ($selected_terms as $term_id) {
                $this->attractionTermClass::firstOrCreate([
                    'term_id' => $term_id,
                    'attraction_id' => $new->id
                ]);
            }
            //Language
            $langs = $this->attractionTranslationClass::where("origin_id", $old->id)->get();
            if (!empty($langs)) {
            ...
            //Meta
            $metaAttraction = $this->attractionMetaClass::where('attraction_id', $old->id)->first();
            if (!empty($metaAttraction)) {
                $metaAttractionNew = $metaAttraction->replicate();
                $metaAttractionNew->attraction_id = $new->id;
                $metaAttractionNew->save();
            }
        }
        ...
        public static function isEnable(){
            return setting_item('attraction_disable') == false;
        }
    }
