...
@section ('content')
    ...
    <div class="language-navigation">
        <div class="panel-body">
            <div class="filter-div d-flex justify-content-between ">
                ...
                <div class="col-right">
                    @if($rows->total() > 0)
                        <span class="count-string">{{ __("Showing :from - :to of :total attractions",["from"=>$rows->firstItem(),"to"=>$rows->lastItem(),"total"=>$rows->total()]) }}</span>
                    @endif
                </div>
            </div>
        </div>
    ...
    @else
        <div class="alert alert-warning">{{__("No attractions found")}}</div>
    @endif
    ...
    <script>
        ...
        $('#items_tab').on('show.bs.tab',function (e) {
            ...
            calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                events:{
                    url:"{{route('attraction.vendor.availability.loadDates')}}",
                    extraParams:{
                        id:lastId,
                    }
                },
        ...
        formModal = new Vue({
            ...
            methods:{
                ...
                saveForm:function () {
                    ...
                    $.ajax({
                        url:'{{route('attraction.vendor.availability.store')}}',
                        data:this.form,
                        dataType:'json',
                        method:'post',
