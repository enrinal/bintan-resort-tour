<div class="bravo-list-item @if(!$rows->count()) not-found @endif">
    @if($rows->count())
        <div class="text-paginate">
            <span class="count-string">{{ __("Showing :from - :to of :total Attractionrs",["from"=>$rows->firstItem(),"to"=>$rows->lastItem(),"total"=>$rows->total()]) }}</span>
        </div>
        <div class="list-item">
            <div class="row">
                @foreach($rows as $row)
                    <div class="col-lg-4 col-md-6">
                        @include('Attractionr::frontend.layouts.search.loop-gird')
                    </div>
                @endforeach
            </div>
        </div>
    ...
    @else
        <div class="not-found-box">
            <h3 class="n-title">{{__("We couldn't find any attractions.")}}</h3>
