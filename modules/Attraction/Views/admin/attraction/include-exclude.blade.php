...
<div class="form-group-item">
    ...
    <div class="g-items">
        @if(!empty($translation->tc))
            @php if(!is_array($translation->tc)) $translation->tc = json_decode($translation->tc); @endphp
            @foreach($translation->tc as $key=>$tc)
                <div class="item" data-number="{{$key}}">
                    <div class="row">
                        <div class="col-md-11">
                            <input type="text" name="tc[{{$key}}][title]" class="form-control" value="{{$tc['title'] ?? ""}}" placeholder="{{__('Eg: Specialized bilingual guide')}}">
                        </div>
                        <div class="col-md-1">
                                <span class="btn btn-danger btn-sm btn-remove-item"><i class="fa fa-trash"></i></span>
                        </div>
    ...
    <div class="g-more hide">
        <div class="item" data-number="__number__">
            <div class="row">
                <div class="col-md-11">
                    <input type="text" __name__="tc[__number__][title]" class="form-control" placeholder="{{__('Eg: Additional Services')}}">
                </div>
                <div class="col-md-1">
                    <span class="btn btn-danger btn-sm btn-remove-item"><i class="fa fa-trash"></i></span>
                </div>
