    ...
    <form action="{{route('car.admin.store',['id'=>($row->id) ? $row->id : '-1','lang'=>request()->query('lang')])}}" method="post">
        ...
        <div class="container-fluid">
            <div class="d-flex justify-content-between mb20">
                <div class="">
                    <h1 class="title-bar">{{$row->id ? __('Edit: ').$row->title : __('Add new merchandise')}}</h1>
                    @if($row->slug)
                        <p class="item-url-demo">{{__("Permalink")}}: {{ url('merchandise' ) }}/<a href="#" class="open-edit-input" data-name="slug">{{$row->slug}}</a>
                        </p>
                    @endif
                </div>
                <div class="">
                    @if($row->slug)
                        <a class="btn btn-primary btn-sm" href="{{$row->getDetailUrl(request()->query('lang'))}}" target="_blank">{{__("View Merchandise")}}</a>
                    @endif
                </div>
            </div>
                            ...
                            <div class="panel">
                                ...
                                <div class="panel-body">
                                    <div class="form-group">
                                        <label>{{__('Merchandise Featured')}}</label>
                                        <br>
                                        <label>
